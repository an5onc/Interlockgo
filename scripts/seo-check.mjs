...
const ignoredDirs = new Set([".git", "node_modules", "_pginfo", "js", "images", "files"]);
...
for (const rel of pages) {
  ...
  if (canonical && canonical !== `https://interlockgo.io${page}`) failures.push(`${rel}: canonical should be https://interlockgo.io${page}`);
  ...
  if (canonical && canonicalUrls.has(canonical)) failures.push(`${rel}: duplicate canonical URL`);
...
const siteText = pages.map((rel) => fs.readFileSync(path.join(root, rel), "utf8")).join("\n").toLowerCase();
for (const keyword of expectedKeywords) {
  if (!siteText.includes(keyword.toLowerCase())) failures.push(`site: missing keyword coverage for "${keyword}"`);
}

const sitemap = fs.readFileSync(path.join(root, "sitemap.xml"), "utf8");
const sitemapUrls = [...sitemap.matchAll(/<loc>(https:\/\/interlockgo\.io\/[^<]*)<\/loc>/g)].map((m) => m[1]).sort();
const expectedUrls = pages.map((rel) => rel === "index.html" ? "https://interlockgo.io/" : `https://interlockgo.io/${path.dirname(rel)}/`).sort();
...
if (!sitemap.includes("<lastmod>2026-05-07</lastmod>")) failures.push("sitemap: May 2026 lastmod not present");
